...
namespace Settle {
    ...
    export class GridForm extends React.Component<BaseDefine.GridFormPropsBase, FormState<Rows, server.Settle>>{
        ...
        noneType() {
            this.gridData(0)
                .done(function (data, textStatus, jqXHRdata) {
                    this.setState({ edit_type: 0, gridData: data, download_src: null });
                }.bind(this))
                .fail(function (jqXHR, textStatus, errorThrown) {
                    CommFunc.showAjaxError(errorThrown);
                });
        ...
        excelPrint(e: React.SyntheticEvent) {
            ...
            var print_url = gb_approot + 'Base/ExcelReport/downloadExcel_SettleData?main_id=' + this.state.fieldData.settle_id + '&t=' + CommFunc.uniqid();
                    ...
                    if (this.state.fieldData.state == SettleState.complete) {
                        buttonHtml = (
                            ...
                            <div className="form-action">
                                <div className="col-xs-10 col-xs-offset-2">
                                    <button type="button" onClick={this.noneType}><i className="fa-times"></i> 回前頁</button> { }
                                    <button type="button" onClick={this.excelPrint.bind(this) } className="btn-success"><i className="fa-print"></i> 列印</button> { }
                                    </div>
                                </div>
                                </div>
                        );
                            ...
                            <div className="form-action">
                                <div className="col-xs-10 col-xs-offset-2">
                                    <button type="button" className="btn-primary"
                                        onClick={this.settleCal.bind(this) }><i className="fa-check"></i> 結算或重新結算</button> { }

                                    <button type="button" className="btn-danger" onClick={this.setSettleState.bind(this) }>
                                        <i className="fa-check"></i> 確認結算<CommCmpt.Tips comment="確認後無法重新結算"/>
                                        </button> { }
                                    <button type="button" onClick={this.noneType}><i className="fa-times"></i> 回前頁</button> { }
                                    <button type="button" onClick={this.excelPrint.bind(this) } className="btn-success"><i className="fa-print"></i> 列印</button> { }
                                    </div>
    ...
    class Detail extends React.Component<DetailProps, DetailState>{
        constructor() {
            ...
            this.render = this.render.bind(this);
            this.state = { details: { rows: [], page: 0, startcount: 0, endcount: 0, records: 0, total: 0 } }
        }
        ...
        loadDetail(page?: number) {
            ...
            CommFunc.jqGet(this.props.apiPath, { settle_id: this.props.MasterId, page: page })
...
ReactDOM.render(<Settle.GridForm caption={gb_caption} menuName={gb_menuname} iconClass="fa-list-alt" />, dom);